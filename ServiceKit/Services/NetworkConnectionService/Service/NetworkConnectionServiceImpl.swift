...
open class NetworkConnectionServiceImpl {
    ...
    private func startConnectionObserving() {
        guard let _ = reachability else {
            return
        }
        
        var context = SCNetworkReachabilityContext.injectedInfo(self)
        let _ = SCNetworkReachabilitySetCallback(reachability!, { (_, flags, info) in
        ...
        let _ = SCNetworkReachabilitySetDispatchQueue(reachability!, queue)
    ...
    private func obtainFlags() -> SCNetworkReachabilityFlags? {
        ...
        guard let _ = reachability, SCNetworkReachabilityGetFlags(reachability!, &flags) else {
            return nil
        }
